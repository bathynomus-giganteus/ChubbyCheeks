...
## 2026-09-08 - 联机修复版本号补正

- 模组版本由 v0.5.01 更新为 v0.5.02，明确区分联机修复前后的工坊下载版本。
- 工坊更新说明仅写“修复部分联机问题”，未添加其他公开说明。

## 2026-09-19 - 开局三种命运选择

- 涅奥开局界面由单一入口改为三个选项：手动五选二、同步随机两种性格、不选择性格并提高高罕度卡牌概率。
- 随机性格与乱码描述使用按玩家和遗物派生的确定性 RNG；卡牌奖励与商店的 20% 高罕度加成使用原版对应 RNG 流。
- 初始遗物新增命运类型、高罕度加成和乱码编号存档字段；`RelicCmd.Replace` 时完整继承到咏春的幸福，升级遗物拾取后的两次稀有卡牌奖励保持不变。
- 第三种命运不写入性格掩码，因此五种性格使徒牌不再被 85% 筛除；遗物描述显示已保存的随机乱码。
- `dotnet build` 成功，0 错误、4 个既有警告；本地游戏模组已更新，工坊订阅目录与上传暂存区暂未覆盖，等待游戏内实测。

## 2026-09-19 - 25 项卡牌与遗物平衡调整

- 完成用户列出的 25 项调整，包含开核桃大师、雪雾、古老的誓约、速邦配送、全心全意、塞兜、捏捏脸、心中的珍珠、战术无人机 MK-2、脑机连接开始、炸猪排市长、偏向性解说、收款时间到了、胡萝卜治愈、向前迈进的决心和两张马约卡牌。
- 重做“要来见少女吗？”为活力/治愈双向半数转换，并使用转换锁阻止递归；埃尔德形态下对应狂热/生命本源。
- 使徒单抽券改为从 10 张同步随机候选中选择 1 张；卡牌单抽券固定获得稀有遗物；金蜡笔阈值改为 4 场战斗；奈雅的海豚水枪改按获得层数累计。
- 粉彩色郊游改为第 X 回合的第 X 张使徒牌费用为 0，重放只计作一次出牌；黄油的黄牌改为事件稀有度，不再进入常规遗物掉落。
- “妖精的监控请求”中英文和日文角色称呼统一为 elf；四种语言的相关卡牌、Power 和遗物描述已同步。
- 32 个本地化 JSON 全部解析通过；`dotnet build` 成功，0 错误、4 个既有警告；本地游戏模组 DLL 与构建产物哈希一致，工坊代码包未覆盖。

## 2026-09-19 - Ronnie Spine 原始素材下载

- 从 Journey Studio 最新 manifest（生成时间 2026-09-14）下载 Ronnie 的 5 套完整 Spine 运行资源到 `E:\work\Cult_leader_mod\SPINE`。
- 已下载基础 Ronnie 与 RonnieSkin1 的战斗模型、正常使徒立绘，以及 RonnieAside；每套均包含 `.skel.bytes`、`.atlas.txt` 和 `.png`，共 15 个文件。
- 文件非空且所有 atlas 引用的贴图均存在；战斗模型和 Aside 为 Spine 4.1.24，正常使徒立绘为 Spine 4.1.08。
- 当前 STS2 使用 Spine 4.2 runtime，因此本轮只保留原始素材，未自动转换，也未覆盖 `SPINE_4_2_TEST` 或发布目录。

## 2026-09-19 - Ronnie Spine 4.2 转换

- 使用现有 `SpineSkeletonDataConverter.exe` 将 Ronnie 的 5 套 Spine 4.1 素材转换为 Spine 4.2.11 JSON。
- 转换结果写入 `E:\work\Cult_leader_mod\SPINE_4_2_TEST`，每套包含 `.spine-json`、`.atlas` 和 `.png`；`SPINE` 中的原始下载文件保持不变。
- 已校验 5 个 JSON 均可解析且版本为 4.2.11，atlas 引用的贴图全部存在；动画数量分别为战斗模型 10、正常使徒立绘 70、Aside 8。
- 本轮只完成转换，尚未接入角色逻辑、同步到本地游戏模组或工坊目录，也未构建和发布。

## 2026-09-19 - 新使徒罗尼与调查终结

- 新增狂热使徒攻击牌 `Apostle_Frenzy_27`【调查终结】：罕见、无费用、不能手动打出、造成 1 点伤害 6 次并消耗。
- 每张卡独立记录当前回合获得活力的次数；第三次获得活力时，若此牌在手牌中，则通过原版 `CardCmd.AutoPlay` 自动打出并使用同步的 `CombatTargets` 随机目标。
- 埃尔德形态下的狂热也按活力事件计数；回合开始时重置计数，计数通过 `SavedProperty` 支持战斗中保存恢复。
- 使用 `E:\work\Cult_leader_mod\坨坨\狂热\罗尼` 中的指定素材：`卡牌.png`、`头像.png`、战斗动作 `Skill1_1`、检视立绘动作 `Tease_2`。
- Ronnie Spine 已加入 `ApostleSpinePrototype`，使用 4.2.11 的 `Ronnie` 战斗模型和正常使徒立绘；头像注册为 `狂热_27`。
- 中、英、日、韩四种卡牌文本均已添加，全部本地化 JSON 解析通过。
- `dotnet build -t:Compile` 成功，0 错误、4 个既有警告；临时 PCK 已确认包含新头像和卡图。
- 当前 `SlayTheSpire2.exe` 正在运行并锁定本地模组 DLL，因此尚未覆盖本地游戏模组，也未提交或发布。

## 2026-09-19 - 罗尼、本轮平衡与表格本地实装

- 【脑机连接开始】最终调整为技能牌，费用 1/0，不再造成伤害；【捏捏脸】最终调整为费用 2/1、抽 2 张牌。
- 第三项“选择混沌的命运”使用原版 `[jitter][red]` 富文本显示红色颤抖标题；咏春的祝福在该命运下扩充为 12 条更混乱的确定性乱码描述。
- 罗尼【调查终结】的代码、卡图、头像与 Spine 资源已完整接入并同步到本地模组、Workshop 订阅目录和 Workshop staging。
- `E:\work\Cult_leader_mod\卡牌信息.xlsx` 已同步本轮 18 个既有卡牌条目、新增罗尼条目、相关遗物与 elf 事件文本；交付副本位于 `D:\OneDrive\codex\outputs\cult-leader-mod-20260919\卡牌信息.xlsx`。
- 本地 `dotnet build` 与 `dotnet build -t:ExportPck` 均成功，0 错误；本地 PCK 已重新导出，DLL 与 `ExportRelease` 产物 SHA-256 均为 `A08A99E97B1A1B4855BA4A3A646EF53B26FAB72CAE22644F3D42D9A9C7B41C5D`。
- 未提交 Git，未执行 Steam Workshop 上传。
