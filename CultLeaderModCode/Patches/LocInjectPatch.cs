	...
	private static void Postfix(string language, Dictionary<string, LocTable> tables)
	{
		...
		if (tables.TryGetValue("cards", out var cardsTable))
		{
			cardsTable.MergeWith(new Dictionary<string, string>
			{
				...
				["CULT_LEADER_MOD_CARD_APOSTLE_PURE_09.description"] = "若再生总量小于{Threshold}层，获得{RegenAmt}层再生；否则触发{TriggerAmt}次再生，并抽{DrawAmt}张牌。",
				...
				["CULT_LEADER_MOD_CARD_APOSTLE_PURE_11.description"] = "造成{Damage:diff()}点伤害。若再生总层数不低于{Threshold}层，减少{Threshold}层并再造成{BonusDamage:diff()}点伤害。",
